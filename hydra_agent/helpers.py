...
def expand_on_basis_of_explicit_representation(response, mappings):
    ...
    The ExplicitRepresentation includes type and language information and 
    differentiates between IRIs and literals by serializing values as follows:
    * IRIs are represented as-is.
    * Literals, i.e., (typed) values and language-tagged strings are represented by their lexical form, 
        surrounded by a single pair of doubles quotes (").
    * If a literal has a language, a single @ symbol is appended after the double-quoted lexical form, 
        followed by a non-empty [BCP47] language code.
    * If a literal has a type, two caret symbols (^^) are appended after the double-quoted literal, 
